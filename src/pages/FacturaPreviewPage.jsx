...
export default function FacturaPreviewPage() {
  ...
  // guardo el URL base (blob:...), y para mostrar en iframe uso un "displayUrl" con #zoom
  ...
  useEffect(() => {
    ...
    async function buildPreview() {
      ...
      const blob = await pdf(<InvoicePdf doc={doc} />).toBlob();
  ...
  const fileName = useMemo(
    () => (doc?.numero ? `FACTURA_${doc.numero}.pdf` : "FACTURA.pdf"),
    [doc]
  );

  const pdfDisplayUrl = useMemo(() => {
    // esto fuerza el visor PDF a usar TODO el ancho disponible
  ...
  if (!doc) {
    return (
      <div style={{ padding: 24 }}>
        ...
        <button onClick={() => navigate("/facturas")}>Volver</button>
      </div>
    );
  }
  ...
  async function exportarElegirRuta() {
    if (canExport()) {
      ...
      else alert("Exportada en: " + res.path);
      return;
    }
  ...
  return (
    <div style={wrap()}>
      ...
      {/* Visor FULL WIDTH + FULL HEIGHT (sin maxWidth) */}
...
function wrap() {
  return {
    ...
    //  importante: ocupa todo el ancho del content
    display: "flex",
