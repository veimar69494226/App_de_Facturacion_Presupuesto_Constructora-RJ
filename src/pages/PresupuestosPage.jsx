...
import { peekNextPresupuestoNumber, nextPresupuestoNumber } from "../storage/docStore";
...
function emptyPresupuesto() {
  ...
  return {
    ...
    tipo: "PRESUPUESTO",
    fecha,
    numero: peekNextPresupuestoNumber("RJ"), // preview
    ...
    notas: "Precios incluyen garantía.\nEste presupuesto tendrá validez de 30 días a partir de su emisión, salvo confirmación expresa de su aprobación dentro de dicho plazo.",
...
function compute(doc) {
  ...
  // subtotal automático tradicional factura
...
/** Solo primera letra en mayúscula (concepto) */
...
function validatePresupuesto(doc) {
  const errors = {};
  const add = (path, msg) => {
  ...
  // validacion para subtotal manual
...
function loadDraftOrEmpty() {
  try {
    ...
    if (!raw) return compute(emptyPresupuesto());

    const parsed = JSON.parse(raw);

    if (!parsed.numeroAsignado) parsed.numero = peekNextPresupuestoNumber("RJ");

    if (!parsed.items || !parsed.items.length) {
      parsed.items = [{ concepto: "", cantidad: 1, precio: 0, totalLinea: 0 }];
    }

    // compat: cliente mínimo
    if (!parsed.cliente) parsed.cliente = { nombre: "" };

    return compute(parsed);
  } catch {
    return compute(emptyPresupuesto());
  }
}
...
export default function PresupuestosPage() {
  ...
  function ensureNumeroReal(d) {
    ...
    const real = nextPresupuestoNumber("RJ");
  ...
  function irAPrevisualizar() {
    ...
    const errs = validatePresupuesto(computedNow);
    setErrors(errs);
    ...
    //  tu ruta de preview de presupuestos (crearemos esa pantalla después)
    navigate("/presupuestos/preview", { state: { doc: finalDoc } });
  }
    ...
    <div style={{ maxWidth: 1100, margin: "0 auto", padding: 16 }}>
      ...
      <div style={{ display: "flex", gap: 12, flexWrap: "wrap", alignItems: "end", marginTop: 10 }}>
        <label style={{ display: "flex", flexDirection: "column", gap: 6 }}>
          <span style={{ fontSize: 12, opacity: 0.85 }}>Nº Presupuesto</span>
          <input value={doc.numero} readOnly style={{ ...styles.input, width: 220 }} />
        </label>
      ...
      <button
        ...
        title={hasErrors ? "Hay campos obligatorios o errores por corregir" : "Generar y previsualizar presupuesto"}
      >
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
        ...
        {/* CLIENTE (solo nombre/razón social) */}
        <div style={{ border: "1px solid rgba(255,255,255,0.10)", borderRadius: 10, padding: 12 }}>
          <h3 style={{ marginTop: 0 }}>Cliente (opcional)</h3>
      ...
      <div style={{ marginTop: 12, border: "1px solid rgba(255,255,255,0.10)", borderRadius: 10, padding: 12 }}>
        ...
        <Field
          ...
          placeholder="Ej: PRESUPUESTO VARIOS: C/LARVA #4"
